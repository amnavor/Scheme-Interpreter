...
def do_if_form(expressions, env):
    ...
    if scheme_true(condition) and condition is not okay:
...
def do_and_form(expressions, env):
    ...
    and_expression = True
    while expressions:
        and_expression = scheme_eval(expressions.first, env)
        if scheme_false(and_expression) or and_expression is okay:
            return False
        expressions=expressions.second
...
def do_or_form(expressions, env):
    ...
    or_expression = False
    while expressions:
        or_expression = scheme_eval(expressions.first, env)
        if scheme_true(or_expression) and or_expression != okay:
...
def check_formals(formals):
    ...
    while formals is not nil:
        if formals.first in valid_list:
            raise SchemeError
        if not scheme_symbolp(formals.first):
            raise SchemeError
        valid_list.append(formals.first)
